...
args.nonlinearityLTP = 1.0063   # nonlinearity in LTP
args.nonlinearityLTD = 0.9138   # nonlinearity in LTD (negative if LTP and LTD are asymmetric)
...
if args.type == 'cifar10':
    ...
    train_loader, test_loader = dataset.get100(
        batch_size=args.batch_size,
        num_workers=1,
        data_root=os.path.join(tempfile.gettempdir(), 'public_dataset','pytorch')
    )
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for name, param in list(model.named_parameters())[::-1]:
                ...
                param.grad.data = wage_quantizer.QG(param.data,args.wl_weight,param.grad.data,args.wl_grad,grad_scale,
                            torch.from_numpy(paramALTP[j]).to("cpu"), torch.from_numpy(paramALTD[j]).to("cpu"), args.max_level, args.max_level)
                j=j+1
