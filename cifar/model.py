...
def mnist(args, logger, pretrained=None):
    """
    A tiny LeNet-ish network over MNIST, but using the same QConv2d/QLinear blocks.
    """
    # first two convolutional stages, with 3→6→16 channels:
    features = nn.Sequential(
        QConv2d(3, 6, kernel_size=5, padding=2, logger=logger, name='Conv1_',
                wl_input    = args.wl_activate,
                wl_activate = args.wl_activate,
                wl_error    = args.wl_error,
                wl_weight   = args.wl_weight,
                inference   = args.inference,
                onoffratio  = args.onoffratio,
        ...
        QConv2d(6, 16, kernel_size=5, padding=0, logger=logger, name='Conv2_',
                wl_input    = args.wl_activate,
                wl_activate = args.wl_activate,
                wl_error    = args.wl_error,
                wl_weight   = args.wl_weight,
    ...
    # now hook into the same CIFAR wrapper, but override its classifier:
    net = CIFAR(args, features, num_classes=10, logger=logger)
    net.classifier = nn.Sequential(
        QLinear(16 * 5 * 5, 120, logger=logger, name='FC1_',
                wl_input    = args.wl_activate,
                wl_activate = args.wl_activate,
                wl_error    = args.wl_error,
                wl_weight   = args.wl_weight,
                ...
                target      = args.target),
        nn.ReLU(),
        QLinear(120, 10, logger=logger, name='FC2_',
                wl_input    = args.wl_activate,
                wl_activate = -1,
                wl_error    = args.wl_error,
                wl_weight   = args.wl_weight,
